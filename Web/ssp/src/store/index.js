...
export default new Vuex.Store({
  ...
  actions: {
    ...
    DELETE_BOOK: async (context, id) => { // имя ,
      if(context.state.isDebug){                            //переменная в stare
        context.commit('DELETE_BOOK', {id:5,ismn:"fdhdfghdg"});//значения  что получаем
      }else {                                                               //
        Vue.axios.delete(`${context.state.BOOKS_API_URL}/byId/${id}`).then((responce) => { //delete - вид запроса, BOOKS_API_URL - переменная staty . then((responce) - ответ
          if (context != null) {
            if (responce.status === 200) {
              context.commit('DELETE_BOOK', responce.data);//вызываю метод DELETE_BOOK из мутецион responce.data получить данные
            }
          }
        }).catch((error) => {
          console.log(error);
